...
class DB_Active_Record extends DB_Driver
{
	...
	private $ar_cache_groupby		= array();
	private $ar_cache_having		= array();
	private $ar_cache_orderby		= array();
	...
	private function _where($key, $value = NULL, $type = 'AND ', $escape = NULL)
	{
		...
		foreach ($key as $k => $v)
		{
            ...
            if(count($this->ar_where) > 0 OR count($this->ar_cache_where) > 0)
            {
                $this->ar_where[] = $type.$k.$v;
            }
            else
            {
                $this->ar_where[] = $k.$v;
	...
	protected function _protect_identifiers($item, $prefix_single = FALSE)
	{
		return $this->_PdoObject->quote($item);
	}
